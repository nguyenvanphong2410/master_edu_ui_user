...
import { setClassOfCourseOfUserSelected, setDataFilterMyCoursesDone, setVisibleModalClassOfCourseOfUser } from '@/states/modules/myCourse';
import { useDebounce } from '@/utils/hooks/useDebounce';
import { requestGetClassOfCourseOfUser, requestGetMyCourseDone } from '@/api/myCourse';
import _ from 'lodash';

export default function TableMyCourseDone() {
  const dispatch = useDispatch();
  const windowWidth = useWindowSize().width;

  
  const myCoursesDone = useSelector((state) => state.myCourse.myCoursesDone);
  const isLoadingMyCoursesDone = useSelector((state) => state.myCourse.isLoadingMyCoursesDone);
  const paginationListCoursesDone = useSelector((state) => state.myCourse.paginationListCoursesDone);
  const dataFilterMyCoursesDone = useSelector((state) => state.myCourse.dataFilterMyCoursesDone);
  
  const debouncedQuery = useDebounce(dataFilterMyCoursesDone.keySearch, 500);

  useEffect(() => {
    dispatch(setDataFilterMyCoursesDone({ ...dataFilterMyCoursesDone, keySearch: debouncedQuery }));
  ...
  const handleChangeTable = (pagination, filters, sorter) => {
    let newDataFilter = _.cloneDeep(dataFilterMyCoursesDone);
    ...
    dispatch(setDataFilterMyCoursesDone(newDataFilter));
    dispatch(requestGetMyCourseDone(newDataFilter));
  }
    ...
    <div>
      <div className={styles.listWrap}>
        <div className={styles.filterWrap}>
          <div className={styles.search}>
            <Input
              ...
              value={dataFilterMyCoursesDone.keySearch}
              onChange={(e) =>
                dispatch(
                  setDataFilterMyCoursesDone({
                    ...dataFilterMyCoursesDone,
        ...
        <div className={'tableWrap h-[calc(100vh-380px)]'}>
          <TableDefault
            loading={isLoadingMyCoursesDone}
            dataSource={myCoursesDone}
            pagination={paginationListCoursesDone}
            isFixed
            ...
            limitTable={dataFilterMyCoursesDone.perPage}
